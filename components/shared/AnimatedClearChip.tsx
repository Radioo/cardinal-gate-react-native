...
export default function AnimatedClearChip({label, height, light, dark, style}: AnimatedClearChipProps) {
    ...
    const containerAnimated = useAnimatedStyle(() => ({
        backgroundColor: interpolateColor(progress.value, stops, palette.bg as string[]),
        borderColor: interpolateColor(progress.value, stops, palette.border as string[]),
    }));

    const textAnimated = useAnimatedStyle(() => ({
        color: interpolateColor(progress.value, stops, palette.text as string[]),
    }));
