...
describe('Pagination', () => {
    ...
    it('renders loading state with disabled buttons', () => {
        const json = JSON.stringify(
            renderer.create(
                <Pagination currentPage={2} totalPages={5} onPageChange={jest.fn()} isLoading/>
            ).toJSON()
        );
    ...
    it('formats large page numbers with locale string', () => {
        const json = JSON.stringify(
            renderer.create(
                <Pagination currentPage={1000} totalPages={5000} onPageChange={jest.fn()}/>
            ).toJSON()
        );
